			...
			<!-- <u-sticky offset-top="18"> -->
				...
				<view class="tabs">
					<u-tabs :list="tabBarsTop" lineColor="#f56c6c" :activeStyle="{
						...
						color: '#606266',
						transform: 'scale(1)'
					}" itemStyle="height: 80rpx;" @click="tabClick">
					</u-tabs>
				</view>
			<!-- </u-sticky> -->
...
<script>
	...
	export default {
		...
		methods: {
				...
				that.$ajax.post({
					...
					if (data.code == 200) {
						for(let i = 0; i < data.data.length; i++){
							...
							that.tabBarsTop.push(item)
						}
						that.requestParams.skill = that.tabBarsTop[0].name
					} 
				});
			},
			//输入框查询
			...
			tabClick(item) {
				if(item.name == "全部"){
					this.requestParams.skill = ''
				}else{
					this.requestParams.skill = item.name
				}
				this.$refs.paging.reload();
			},
			gotoOrderDetail() {
